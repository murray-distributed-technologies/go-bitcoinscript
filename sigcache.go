...
func (s *SigCache) Exists(sigHash chainhash.Hash, sig *bsvec.Signature, pubKey *bsvec.PublicKey) bool {
	s.RLock()
...
func (s *SigCache) Add(sigHash chainhash.Hash, sig *bsvec.Signature, pubKey *bsvec.PublicKey) {
	s.Lock()
	defer s.Unlock()
