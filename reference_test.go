...
func parseExpectedResult(expected string) ([]ErrorCode, error) {
	...
	case "DIV_BY_ZERO":
		return []ErrorCode{ErrDivByZero}, nil
	case "MOD_BY_ZERO":
		return []ErrorCode{ErrModByZero}, nil
	case "PUBKEYTYPE":
	...
	case "SCRIPTNUM_OVERFLOW":
		return []ErrorCode{ErrScriptNumOverflow}, nil
	case "SPLIT_RANGE":
		return []ErrorCode{ErrSplitRange}, nil
	case "OPERAND_SIZE":
		return []ErrorCode{ErrOperandSize}, nil
	case "ELEMENT_SIZE":
		return []ErrorCode{ErrElementTooBig}, nil
	case "PUBKEY_COUNT":
...
func testScripts(t *testing.T, tests [][]interface{}, useSigCache bool) {
	...
	for i, test := range tests {
		...
		// error codes.  This is necessary because bitcoinscript is more
		...
		if resultStr == "OK" {
			if err != nil {
				t.Errorf("%s failed to execute: %v; vm: %#v", name, err, vm.GetStack())
			}
			continue
		}
