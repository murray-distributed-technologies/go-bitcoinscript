...
// An opcode defines the information related to a bitcoinscript opcode.  opfunc, if
...
func opcodeCheckLockTimeVerify(op *parsedOpcode, vm *Engine) error {
	...
	// value is before the bitcoinscript.LockTimeThreshold.  When it is under the
...
func opcodeSplit(op *parsedOpcode, vm *Engine) error {
	...
	if n.Int32() > int32(len(c)) || n < 0 {
		return scriptError(ErrSplitRange, "n is outside range of input length")
	}
	a := c[:n]
	b := c[n:]
...
func opcodeAnd(op *parsedOpcode, vm *Engine) error {
	...
	if len(a) != len(b) {
		return scriptError(ErrOperandSize, "byte arrays are not the same length")
	}
	c := make([]byte, len(a))
	for i := range a {
		c[i] = a[i] & b[i]
...
func opcodeDiv(op *parsedOpcode, vm *Engine) error {
	...
	if b == 0 {
		return scriptError(ErrDivByZero, "divide by zero")
	}
...
func opcodeMod(op *parsedOpcode, vm *Engine) error {
	...
	if b == 0 {
		return scriptError(ErrModByZero, "mod by zero")
	}
	vm.dstack.PushInt(a % b)
	return nil
}

// BSV changed this to be a bitwise shift, not a numeric shift
// TODO: This opcode is broken over byte boundaries right now. Also does not
// handle large byte arrays
func opcodeLShift(op *parsedOpcode, vm *Engine) error {
	...
	if n.Int32() < 0 {
		return scriptError(ErrInvalidNumberRange, "n less than 0")
	}
...
/*func ShiftLeft(dst, src []byte, bits int32) {
	bits &= 0x7
	trunc := 8 - bits
	last := len(src) - 1
	for i := 0; i < last; i++ {
...
func ShiftLeft(src []byte, bits int32) error {
	if bits == 0 {
		return nil
	}
	if bits > int32(len(src)*8) {
		for i := 0; i < len(src); i++ {
			src[i] = byte(0)
		}
		return nil
	}
	n := bits / 8  //  bytes to zero out
	m := bits % 8  // remainder number of bits to shift second to last non-zero byte
	trunc := 8 - m // bits to truncate from i+1
	m &= 0x7

	for i := 0; int32(i) < int32(len(src))-n-1; i++ {
		src[i] = src[i]<<m | src[i+1]>>trunc
	}

	lastPartial := int32(len(src)-1) - n
	if lastPartial < 0 {
		lastPartial = 0
	}
	src[lastPartial] = src[lastPartial] << m
	for i := lastPartial; i < int32(len(src)-1); i++ {
		src[i] = byte(0)
	}
	if m == 0 {
		src[lastPartial] = byte(0)
	}
	return nil
}

func ShiftRight(src []byte, bits int32) error {
	// Nothing to shift
	if bits == 0 {
		return nil
	}
	// shift amount is higher than length of bits
	if bits > int32(len(src)*8) {
		for i := 0; i < len(src); i++ {
			src[i] = byte(0)
		}
	...
	n := bits / 8  //  bytes to zero out
	m := bits % 8  // remainder number of bits to shift
	trunc := 8 - m // bits to truncate from i-1
	m &= 0x7

	// Loop from end of array to first element
	last := len(src) - 1
	for i := last; int32(i) > n; i-- {
		src[i] = src[i]>>m | src[i-1]<<trunc
	}

	lastPartial := n
	if lastPartial > int32(len(src)-1) {
		lastPartial = int32(len(src) - 1)
	}
	src[lastPartial] = src[lastPartial] >> m
	for i := lastPartial; i > 0; i-- {
		src[i] = byte(0)
	}
	if m == 0 {
		src[lastPartial] = byte(0)
...
func opcodeRShift(op *parsedOpcode, vm *Engine) error {
	...
	if n > 520 {
		return scriptError(ErrInvalidNumberRange, "n is greater than max element size")
	}
