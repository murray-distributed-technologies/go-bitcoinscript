...
// newAddressPubKey returns a new bsvutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) bsvutil.Address {
	addr, err := bsvutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new bsvutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) bsvutil.Address {
	addr, err := bsvutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new bsvutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) bsvutil.Address {
	addr, err := bsvutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the bsvutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the bsvutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := bsvutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2pkCompressedMain, err := bsvutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkUncompressedMain, err := bsvutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*bsvutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bsvutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*bsvutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*bsvutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*bsvutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
